...
export default function Precos() {
  ...
  useEffect(() => {
    const createBubble = () => {
      const background = document.querySelector('.background'); if (!background) return;
      const bubble = document.createElement('div'); bubble.className = 'bubble';
      const size = Math.random() * 100 + 50; const left = Math.random() * 100;
      bubble.style.width = `${size}px`; bubble.style.height = `${size}px`; bubble.style.left = `${left}%`;
      bubble.style.animationDuration = `${Math.random() * 10 + 15}s`;
      background.appendChild(bubble); setTimeout(() => bubble.remove(), 20000);
    };
    const interval = setInterval(createBubble, 1000); return () => clearInterval(interval);
  }, []);
          ...
          <table className="w-full">
            <thead className="bg-black/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold">Preço {usarPrecoParceria ? 'Parceria' : 'Normal'}</th>
              ...
              {servicos[categoriaSelecionada].items.map((item, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">{precoStr}</td>
